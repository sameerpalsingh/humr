...
/**********----- Old Queris---------------------********************//////////
/* $ss="SELECT hmb.*, hm.loginid as loginid
                         FROM hum_messagebox hmb, hum_members hm
                         WHERE hmb.sentby='".$_SESSION['sess_user_id']."'
                         AND hmb.sentby=hm.id
                         ORDER BY message_id desc";
						 
$ss="select * from hum_member_contact where contact_id ='".$_SESSION['sess_user_id']."'";
$rs = $db->executeQuery($ss);
*//*********************************************************************//////////
$sql = mysqli_query($link, "select * from hum_inbox where sentby ='".$_SESSION['sess_user_id']."' order by message_id desc");
...
$a="select * from hum_inbox where sentby ='".$_SESSION['sess_user_id']."' order by message_id desc limit $from,$show_record_no";
      ...
      <tr>
        ...
        <td valign="top"><?php include(DIR_FS_INCLUDES."header.inc.php"); ?>
           <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td width="30">&nbsp;</td>
                <td valign="top"><?php include(DIR_FS_TEMPLATES."sent_box.tpl.php");?></td>
                <td width="30">&nbsp;</td>
              </tr>
              <tr>
                <td>&nbsp;</td>
