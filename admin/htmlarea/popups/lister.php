...
#BOF DSJ Added AutoThumbnail Feature CNSTANTS
define('THUMB_HEIGHT', 144); 
define('THUMB_WIDTH', 144); 
define('CREATE_THUMB', true); 
#define('THUMB_DIR', 'thumbs/'); //relative
#EOF DSJ Added AutoThumbnail Feature CNSTANTS 
?>
<style type="text/css">
   ...
   if(!(file_exists(IMAGE_DIR . $path))) {
      ...
      if(!(@mkdir(IMAGE_DIR . $path, 0777)))
         ...
         $error = "Folder \'" . $path . "\' could not be created";
	   		 ...
	   		 $error = "Folder \'" . $path . "\' created sucessfully";
	   		//@mkdir(IMAGE_DIR. THUMB_DIR . $path, 0777)
	   		}
   }
         ...
         if($search($types[$type], $name)) {
            ...
            if(!(file_exists(IMAGE_DIR . $path))) {
               ...
               if(UPLOAD_LIMIT == 0 || $size <= UPLOAD_LIMIT) {
				  ...
				  //if(CREATE_THUMB)
				  //{
				  	$im_file_name = IMAGE_DIR . $path;
					$error =$im_file_name;
					$temp = substr($im_file_name,0,strlen($im_file_name)-4);
					$ext1 = substr($im_file_name,strlen($im_file_name)-4,10);
				   if(CREATE_THUMB && $ext1=='jpg')	
				   {$th_file_name = $temp.'_thumb'.$ext1;
					$error .= $th_file_name;
				  	$image_attribs = getimagesize($im_file_name); 
					$im_old = imageCreateFromJpeg($im_file_name); 
				  	$th_max_width = THUMB_WIDTH; 
					$th_max_height = THUMB_HEIGHT; 
					...
					//$th_file_name = IMAGE_DIR . THUMB_DIR . $path; 
...
function fileTag($value, $key, $depth) {
   ...
   if($size) {
      ...
      echo $value;
      echo "</a>\n";
/*ds echo scriptURL("viewer.php?DPI=" . AGENT_DPI . "&file=" . urlencode($file)); */
      echo "</td><td class=\"delete\" align=\"right\" valign=\"bottom\">\n";
      if(SUPPORT_DELETE) {
